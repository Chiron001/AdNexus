...
function checkConversionTrackingBroken(
  ...
  if (totalConversions === 0 && totalSpend > 10000) {
    issues.push({
      ...
      description: `Your account has spent ₹${Math.round(totalSpend).toLocaleString('en-IN')} in the last 30 days but recorded zero conversions. Your conversion tracking tag may be firing incorrectly or not at all.`,
  ...
  return [
    {
      ...
      description: `${lowQsKeywords.length} keywords have a Quality Score of 4 or below and have spent ₹${Math.round(totalWasted).toLocaleString('en-IN')}. Low Quality Scores mean you pay more per click than competitors for the same position.`,
  ...
  return [
    {
      ...
      severity: 'high',
      title: `${wastedKeywords.length} keywords spending ₹${Math.round(totalWasted).toLocaleString('en-IN')} with zero conversions`,
      description: `${wastedKeywords.length} keywords have each spent over ₹5,000 in the last 30 days but generated zero conversions. These keywords need to be paused or have their match types tightened.`,
      affected_entity_type: 'keyword',
  ...
  for (const campaign of campaigns) {
    ...
    if (conversions === 0) {
      issues.push({
        ...
        severity: 'high',
        title: `Campaign ROAS critically low — ₹${Math.round(spend).toLocaleString('en-IN')} at risk: "${campaign.campaign.name}"`,
        description: `This campaign has spent ₹${Math.round(spend).toLocaleString('en-IN')} in the last 30 days with zero recorded conversions. The return on ad spend is effectively zero.`,
        affected_entity_type: 'campaign',
        affected_entity_id: campaign.campaign.id,
        affected_entity_name: campaign.campaign.name,
