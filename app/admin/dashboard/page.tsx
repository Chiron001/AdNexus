...
const EVENT_META: Record<string, { label: string; cls: string }> = {
  'subscription.charged':   { label: 'Charged',    cls: 'bg-green-500/20 text-green-400'    },
...
const CARD = 'rounded-xl p-5 transition-all'
const CARD_STYLE = {
...
export default async function AdminDashboardPage() {
  ...
  const STATS = [
    ...
    { label: 'Open Issues', value: String(allIssues.length),  sub: `${criticalCount} critical · ${highCount} high`, accent: criticalCount > 0 ? 'text-red-400' : 'text-zinc-500', href: '/admin/activity' },
  ]
    ...
    <div className="p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {STATS.map(s => (
          <Link
            ...
            style={CARD_STYLE}
            onMouseEnter={e => { Object.assign((e.currentTarget as HTMLElement).style, CARD_HOVER_STYLE) }}
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
        ...
        <div className={CARD} style={CARD_STYLE}>
          ...
          <div className="mt-4 pt-4 flex justify-between text-xs" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {(recentSignups ?? []).map(u => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-3 -mx-2 px-2 py-1.5 rounded-lg transition-colors"
                  style={{ background: 'transparent' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.05)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent' }}
                >
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ background: 'rgba(124,58,237,0.25)', color: '#a78bfa' }}>
                    {initials}
                  </div>
