...
export function BudgetCharts({
  dailySpend, cpmTrend, usedPlatforms, sym = '$',
}: {
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={dailySpend}>
            ...
            <YAxis tickFormatter={v => `${sym}${(v/1000).toFixed(0)}k`} tick={{ fill: '#71717a', fontSize: 10 }} axisLine={false} tickLine={false}/>
            <Tooltip contentStyle={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: 8, fontSize: 12 }} formatter={(v: any) => [`${sym}${(v/1000).toFixed(1)}k`, 'Spend']}/>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={cpmTrend}>
            ...
            <XAxis dataKey="day" tick={{ fill: '#71717a', fontSize: 10 }} axisLine={false} tickLine={false}/>
            <YAxis tickFormatter={v => `${sym}${v}`} tick={{ fill: '#71717a', fontSize: 10 }} axisLine={false} tickLine={false}/>
            <Tooltip contentStyle={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: 8, fontSize: 12 }} formatter={(v: any) => [`${sym}${v}`]}/>
            {usedPlatforms.map(p => (
